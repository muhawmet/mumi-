---
name: mamilas-upwork-portfolyo-hedefi
description: "Zincirin sonu Upwork portfolyosu — reklam filmi bir deneme değil, satılabilir demo"
metadata: 
  node_type: memory
  type: project
  originSessionId: 97e409ef-fb0e-4fa2-82d7-310ae053a13f
  modified: 2026-07-27T20:20:56.939Z
---

Mami (2026-07-27): *"En sonunda da Upwork için portfolyoluk videolar yapalım, demoları oraya
yaparız."*

**Why:** MAMILAS'ın hedefi eğitim videosundan reklam filmine döndü ([[mamilas-mami-kisisel]] —
şirket MAMILAS'ı bilmiyor, bu onun kendi malı). Zincirin sonu iç bir yetenek kanıtı değil:
**dışarıya satılan bir demo reeli.** Bu, FAZ 2'nin çıktısını değiştirir — ilk `product_brand_real`
filmi bir deney değil, portfolyo parçasıdır ve konu/marka seçimi buna göre yapılır.

**How to apply:** üretim kararlarını "bu kare bir müşteriye gösterilir mi" ölçütüyle ver. Demo
markasız olabilir (o zaman `brandKitLock` engel değil); marka konacaksa önce kilidin giriş yüzeyi
gerekir — kilit boşken ölçülen gerçek çıktı müşterinin kendi logosunu üç kez yasaklıyor.
İlgili: [[mamilas-uretim-akisi]] · [[mamilas-v2-kutuphane-makro]]
