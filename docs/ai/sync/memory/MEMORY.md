...
- [Claude senkronu — Mac ↔ Windows](mamilas-claude-senkronu.md) — akıl `~/.claude`'da yaşar, git taşımaz. **Tek kanon `claude-sync.mjs`**: iki yönlü, asla silmez, çatışmada durur. `memory-sync.mjs` 2026-08-05'te SİLİNDİ (emekli bırakmak = bir gün koşmak).
- [MAMILAS OS — beş organ](mamilas-os-bes-organ.md) — otonom stüdyonun mimarisi: brifing kapısı · iş emri omurgası · rota (motor+cüzdan) · üç kademeli göz · kurgu zekâsı · karar yüzeyi. Ölçülen dört bulgu içeride.
