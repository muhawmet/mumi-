...
7. Motion seed: the frame is the half-second before this event.
...
12. Final line: "Clean motion-ready start frame".
</prompt_recipe>
...
4. Per-style drawing grammar to reinforce in prompt phrasing:
   ...
   - `jjk_ink_style` → abstract fractal-flame energy (non-IP), MAPPA ink smear at peak, cinematic dark
...
leaks across path, if IP is copied, if the frame is not ready for motion, if palette became flat fills, or if an IP
