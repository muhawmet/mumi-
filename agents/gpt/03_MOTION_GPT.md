...
2. One moving element, one cause-effect-settle event, stable final hold.
...
5. If duration exceeds the clean window, split into balanced shots. If the brief
   gives no model window, assume about 9 seconds. Each split needs its own
...
`[ID] MOTION (I2V plays approved start frame)`  
Camera. Moving element. Event. Rhythm. Locked-still elements. Final hold.
Negative failure modes.

## Gate

Reject new objects, re-rendering, style/material drift, two unrelated actions,
mouth movement without approval, logo/text warp, no final hold, or bad split math.
