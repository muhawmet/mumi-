...
export function derle(projeAd, { sekans = null } = {}) {
  ...
  const govde = [
    ...
    '> 🔴 ÜRETİM ZİNCİRİ: sen YAZARSIN → orkestratör KONTROL EDER → orkestratör BASAR.',
    '> Kare BASMAZSIN. MCP üretim çağrısı ajanda kapalıdır ve bu bir rica değil —',
    '> `harcama-kapisi.mjs` `agent_id` görünce reddediyor (canlı ölçüldü 2026-08-07: `[AJAN]`).',
    '> Yazdığın prompt OKUNACAK: dünya kaynaktan mı · `@handle` çağrılmış mı (tarif değil) ·',
    '> kadrajın önünde odak dışı kütle var mı · negatif sahneye özel mi · ekranda yazı var mı.',
