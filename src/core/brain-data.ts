...
export const EDU_BANK: Bank =[
 ...
 [/\bkalp\b|karıncık|kan dolaşım|damar|pompal|kardio|heart|cardiac|ventricle|kalp at[iı][sş]/i,
  'one cutaway heart model with two visible chambers, inflow and outflow vessels clearly labelled',
  'one side of the heart contracts once, the blood flow arrow travels its complete loop from heart to body and back, and the model settles at rest'],
 [/sinir sistem|beyin|n[oö]ron|refleks|n[oö]ral|beyin sap[iı]|h[uü]cre g[oö]vde|akson|dendrit/i,
  'one cross-section brain model with a single lit neuron pathway and its target muscle token',
  'the neuron signal sparks once from cell body along the axon, crosses the synapse, and the muscle token contracts and relaxes in one response'],
 [/elektrik|devre|pil|amp[uü]l|ak[iı]m|volt|anahtar|iletken|direnc/i,
  'one clear circuit board with a battery token, a switch, and a small bulb at the far end',
  'the switch closes once, the current path lights amber across the board, the bulb glows steady, and the circuit settles lit'],
 [/yans[iı]ma|k[iı]r[iı]lma|prizma|g[oö]r[uü]nt[uü] olu[sş]|optik|lens.*[iı][sş][iı]k|[iı][sş][iı]k.*k[iı]r[iı]l/i,
  'one tabletop optics rig: a lamp, a glass prism on a pivot, and a white screen',
  'the lamp beam enters the prism once, bends into its spectrum band on the screen, and the prism settles showing the full colour spread'],
 [/\bses\b|titreşim|dalga|yankı|ses[iş] hız[iı]|sesi [iı]let|frekans|akustik/i,
  'one sound-table with a vibrating tuning fork and a shallow water tray showing concentric rings',
  'the fork strikes once, the rings spread from the contact point outward across the tray and fade to calm'],
 [/madde|element|bile[sş]ik|atom|molek[uü]l|kimyasal|peryodik|reaksiyon|yan[iı]c[iı]|asit|baz/i,
  'one element-tile board with atom-model tokens and a bonding track between two element slots',
  'two atom tokens slide into the bonding track once, bond lines click into place, and the compound tile settles readable at the output end'],
 [/\b[uü][cç]genin a[cç][iı]lar[iı]|\biç a[cç][iı]\b|dik a[cç][iı]|a[cç][iı] [oö]l[cç]|a[cç][iı] toplam[iı]/i,
  'one angle table with three movable arm rods and a central protractor disc',
  'the three arms hinge to their positions one by one, the disc reads the inner angle sum, and all three settle locked at the proof value'],
 [/[uü]retim|tar[iı]m|[cç]ift[cç]i|ekim|hasat|toprak|tohumun b[uü]y|bitki ye[tş]i[sş]|tar[iı]msal/i,
  'one growth-cycle planting stage: a transparent pot showing soil layers, a seed, and a root system forming',
  'the seed swells and sends its first root downward through the visible soil layers in one continuous grow, the shoot rising to the surface'],
 [/h[oö]caman b[uü]t[uü]n|ortak kat|en b[uü]y[uü]k ortak b[oö]len|OBOB|OKEK|lcm|gcd/i,
  'one factor grid where two number tiles share a highlighted column of common factors',
  'the grid lights the shared factor column amber once, one factor tile rises to mark the answer, and the grid settles on the confirmed value'],
 [/h[uü]cre b[oö]l[uü]nm|mitoz|mayoz|DNA|gen|kal[iı]t[iı]m|kromozom|[oö]zell[iı]k kal[iı]t/i,
  'one cell dome that splits cleanly into two equal daughter cells on a clean surface',
  'the cell dome pinches at its equator once, splits into two equal halves that drift apart, each inheriting the same inner chromosome pattern'],
...
export const STY_BANK: Bank =[
 ...
 [/intikam|revenge|öfke.*derin|\bkin\b|vendetta|rage.*coil/i,
 ...
 [/g[uü][cç]len|g[uü][cç] kazan|g[uü][cç] y[uü]ksel|power.*surge|enerji patlamas[iı]|aura.*rise|power.*level|y[uü]ksel[iı][sş]|awakening|ascent|dungeon.*gate|gate.*open/i,
 ...
 // --- Sci-fi / mecha / space ---
 [/robot|mecha|siborg|mekanik kalp|yapay zeka|biyonik|exo.?suit|cyborg|android/i,
 ...
 [/uzay|galaksi|uzay istasyonu|uzay yolculu[gğ]u|galaktik|uzay sava[sş]|yıldız|nebula|evren|orbital|planet|güne[sş] sistemi/i,
 ...
 [/ejderha|b[uü]y[uü]c[uü]|b[uü]y[uü]l[uü]|b[uü]y[uü]s[uü]|fantezi|krall[iı]k|sihir|elfler|c[uü]celer|mitoloji/i,
...
export const REAL_BANKS: Record<string, Bank> ={
 PRODUCT:[
  ...
  [/ekran a[cç][iı]l[iı][sş]|aray[uü]z|interface|screen open|ui appear|uygulama|app launch/i,'the product screen waking in its real context, UI geometry settling crisp and legible, no stock glow','the screen wakes once to the first UI state, glass reflection controlled, and the interface settles readable and exact'],
  [/zafer|kazanma an[iı]|ba[sş]ar[iı]|son nefes|finish|victory hold|final stand/i,'the athlete or subject in the held beat after the peak — the stance, the breath, the earned stillness','the motion has already completed; the figure holds in the honest aftermath of effort, one slow breath the only movement, and the frame settles on what has just been won'],
  ...
  [/atlet|sporcu|ko[sş]u|f[iı]nish|zafer|kazan|gol|depar|performans|champion|win|victory/i,'the athlete at the decisive instant — finish line, peak effort, the held breath before result','the peak motion completes once at documentary speed, the athlete\'s silhouette finding its true line, and the frame holds on the unforced truth of the achieved moment'],
  [/kampanya|dünya etkile|küresel|marka gücü|vizyon|devrim|brand power/i,'the brand mark or icon in its real environment at a scale that earns respect','the brand light finds its mark once as the environment holds authentic and vast, and the frame settles on a quiet claim that needs no caption']
 ],
 FOOD:[
 ...
 CIVIC:[
  ...
  [/i[sş][cç]i|[cç]al[iı][sş]an|esnaf|d[uü]kkan|sabah[iı]|erken sabah|sokak hayat|daily|worker|morning city/i,'the working person at their honest daily task, city texture in the background, light municipal and practical','one real work beat completes — a crate lifted, a stall opened, a door unlocked — in documentary rhythm, the city waking in the background unaffected'],
  [/pazar|tezgah|market|manav|seyyar|hamal/i,'the market stall in morning setup, produce and surface texture honest, one worker action the center','one stall-building beat completes — a crate set, a cloth laid, a price card placed — and the market settles in the honest disorder of preparation'],
  [/[oö]grenci|okul|[cç]ocuk duraks|lise|s[iı]n[iı]f|school|student/i,'the school-age child at a civic threshold — gate, crossing, corridor — city texture grounding the frame','one unhurried school beat completes — a bag adjusted, a door pushed, a step taken — and the child settles in the ordinary morning before learning begins']
 ],
 EVENT:[
  ...
  [/[sş]irket|ofis|[sş]irket binas[iı]|kurumsal|corporate|headquarters|genel merkez/i,'the company building or headquarters at honest architectural scale, real light, no stock perfection','one moment of real institutional life completes — a door opened, a light coming on, a figure crossing — and the building settles as a place where work actually happens'],
  [/karar|toplant[iı] odas[iı]|masa ba[sş][iı]|y[oö]netim|leadership|board|executive/i,'the decision environment at honest scale — table, chairs, materials — no one in heroic pose','one deliberate decision-table beat completes — documents exchanged, a chair drawn, a nod — and the room settles in the weight of what was just decided']
 ],
 TESTIMONIAL:[
  ...
  [/mahallesi|toplulu[gğ]u|s[oö]z al|taahhüt|s[oö]z verir|pledge|commit|community stand/i,'the person standing in their real community space, environment providing the proof','one commitment beat completes — a gesture toward the neighborhood, a hand extended, a direct look — and the person settles in the honest weight of the promise they just made']
 ],
 FASHION:[
  [/moda|manken|\bmodel\b|k[iı]yafet|giyim|stil|trend|tasar[iı]mc[iı]|butik|elbise|giysi|sezon|l[uü]ks giyim|fashion|outfit|look/i,'the model as a sculpted silhouette against deep negative space, one editorial light edge defining the line','the model turns one deliberate degree, the light edge re-carving the silhouette as fabric weight follows late, and the stance settles in held authority'],
  ...
  [/sahneye gir|giri[sş]|[iı][sş][iı]k y[uü]z|gir|enter|arrive/i,'the model entering the real set at walking pace, light landing on the face as they cross the threshold','the model takes the first deliberate step into the key, the face catching light for the first time, and the entrance settles with the editorial gravity of a first look'],
  [/aksesuar|tak[iı]|yüzük|kolye|bileklik|accessory|jewel/i,'the accessory at macro editorial distance, one precise surface and material truth the entire argument','one controlled key event crosses the accessory — metal edge, gem facet, leather grain — and it settles in held luxury, the rest of the frame disciplined to absence']
 ],
 TOURISM:[
  ...
  [/yelken|ufukta|yeni toprak|yeni d[uü]nya|gemi|horizon|voyage|sail|discover/i,'the ship or vessel on open water, horizon unbroken behind, the scale of arrival visible in a single frame','the bow lifts one degree into the light as the horizon line holds vast and steady, and the frame settles on the emotion of new distance']
 ],
 AUTO:[
 ...
 ARCH:[
  ...
  [/mermer|granit|tahta|parke|m[uü]kemmel|y[uü]zey sun|surface perfect|material detail/i,'the material surface at architectural macro distance, finish honest, light raking the plane to reveal depth','one raking light band crosses the material surface once, micro-relief confirmed, and the finish settles in the quiet authority of true craft']
 ],
 SOCIAL:[
  ...
  [/arkada[sş]|bulu[sş]|bir arada|g[uü]ne[sş]li|friends|gather|together|group/i,'real friends in outdoor or casual setting, faces natural, location texture doing the warmth','one genuine social beat completes — a laugh, a gesture, a shared glance — in natural handheld rhythm, the group settling in unforced connection'],
  [/kafe|kahvalt[iı]|masa ba[sş][iı]|otur|an|enjoy|cafe|coffee moment/i,'real people at a café or table, the social moment and the place in equal frame','one comfortable table beat completes — a cup lifted, a word shared, a small gesture — and the café moment settles thumb-stop warm']
 ],
 HEALTH:[
