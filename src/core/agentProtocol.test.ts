...
describe('agent protocol — deterministic hash/tamper/stale', () => {
  test('aynı artifact aynı contentHash üretir; protocol hash SHA-256', () => {
    const a = createAgentArtifact({ ...base, phase: 'IMAGE_PROMPT', role: 'image_author', content: { prompt: 'x' } });
    const b = createAgentArtifact({ ...base, phase: 'IMAGE_PROMPT', role: 'image_author', content: { prompt: 'x' } });
  ...
  test('REJECT exact check+targeted fix; FACT_REQUIRED eksik gerçeği taşır', () => {
    ...
    const fact = createAgentArtifact({ ...base, phase: 'IMAGE_JURY', role: 'image_jury', content: { verdict: 'FACT_REQUIRED', factRequired: 'ürün geometrisi' } as JuryContent });
...
describe('on-demand lifecycle — bir author, bir jury, en fazla bir revision', () => {
  const author = (revision: 0 | 1) => createAgentArtifact({ ...base, revision, phase: 'IMAGE_PROMPT', role: 'image_author', content: { prompt: `p${revision}` } });
  const jury = (verdict: JuryContent['verdict'], revision: 0 | 1, extra: Partial<JuryContent> = {}) => createAgentArtifact({
    ...base, revision, phase: 'IMAGE_JURY', role: 'image_jury', content: { verdict, ...extra } as JuryContent,
  });
  ...
  test('APPROVE olmayan frame motion açmaz', () => {
    ...
    expect(nextLifecycleAction(chain, { verdict: 'REGENERATE', frameHash: 'f' })).toEqual({ kind: 'AWAIT_MAMI_APPROVE' });
    expect(nextLifecycleAction(chain, { verdict: 'APPROVE', frameHash: 'f' })).toEqual({ kind: 'RUN_ROLE', role: 'frame_jury', revision: 0 });
  });
});
