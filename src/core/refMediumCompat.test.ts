...
describe('madde 18 — worldId olmayan ref evrensel DEĞİL: medium uyumu ölçülür', () => {
  ...
  // B1+B2 kök fix (2026-07-17, Mami kararı): pinli ref artık YALNIZ kendi dünyasında.
  // Eskiden CINEMATIC_REAL→CINEMATIC_REAL çaprazı vardı — o çapraz Tarkovsky'nin
  // "Soviet 70s / long-take drift"ini başka bir real dünyaya taşıyordu (konum+hareket sızıntısı).
  it('B1/B2: pinli CINEMATIC_REAL ref BAŞKA CINEMATIC_REAL dünyaya artık GEÇMEZ (konum/hareket sızıntısı kapandı)', () => {
    ...
    expect(refCompatibleWithWorld(cinematicPinned, otherReal.id)).toBe(false);
    // ama kendi dünyasında hâlâ geçerli:
    expect(refCompatibleWithWorld(cinematicPinned, cinematicPinned.worldId!)).toBe(true);
  });

  it('B1: Tarkovsky (chivo home) deakins sahnesine Soviet DNA taşıyamaz', () => {
    const tark = DATA.refs.find((r) => r.id === 'tarkovsky_slow_nature');
    if (!tark) return;
    expect(refCompatibleWithWorld(tark, 'deakins_naturalist')).toBe(false);
  });
});
